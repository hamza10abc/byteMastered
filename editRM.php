<?php 
    ...
    if ($_SERVER['REQUEST_METHOD'] =='POST'){
        ...
        $sql = "UPDATE `raw_material` SET `name` = '$name', `old_rate` = '$old_rate', `final_rate` = '$final_rate' WHERE `raw_material`.`_id` = $idRM";
        $result= mysqli_query($conn, $sql);
        if($result){
            $update = true;
        }
    ...
    <?php
        if($update){
            ...
            <div class='flex items-center p-4 mb-4 text-sm text-green-800 border border-green-300 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400 dark:border-green-800' role='alert'>
                ...
                <div>
                    <span class='font-medium'>Success alert!</span> Data has been Updated succesfully. -->
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div class="bg-white border border-gray-100 shadow-md shadow-black/5 p-6 rounded-md lg:col-span-3">
            ...
            <div>
                ...
                <form action='editRM.php?id=<?=$idRM?>' method="post">
                    <div class="mb-6">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="number" step="any" step="any" id="rate" name="old_rate" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="<?=$old_rate?>" required />
                    </div>
                    <div class="mb-6">
                        <label for="final_rate" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Final Rate</label>
                        <input type="number" step="any" step="any" id="final_rate" name="final_rate" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="<?=$final_rate?>" required />
                    </div>
